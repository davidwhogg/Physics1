...
Due Tuesday 2025 December 9 by 23:59 NYC time on Brightspace.
Be sure to give credit to your sources, as per our credit policies.
...
\paragraph{\problemname~\theproblem:}\refstepcounter{problem}%
From the notes at \url{http://cosmo.nyu.edu/hogg/sr/},
Problem 4--11, except make the following changes: First,
make the Lorentz factor exactly $\gamma=7$. Second, set $\ell$
such that the traveling twin (Lin) turns 24 on the day she
turns the ship around. That is, set the journey to take 4 years
each way (8 years total) for Lin.
...
trajectory on an $x$ \textsl{vs} $t$ spacetime diagram is a hyperbola, where both
...
\textsl{(c)}~\textbf{(Extra optional part, will not be graded for credit:)}
Show that this trajectory is unchanged under the Lorentz
